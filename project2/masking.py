...
def mask(input_img, input_anno, output_img, output_anno):
    ...
    with open(input_anno, 'r') as f:
        ...
        for l in lines:
            ...
            elif float(box_data[3]) < 40 / img.size[0] and float(box_data[4]) < 40 / img.size[1]:
                mask_box += [[float(b) for b in box_data[1:]]]
            else:
                if box_data[0] in ['5', '6', '7', '8', '9', '10']:
                    box_data[0] = '5'
                    l = box_data[0] + ' ' + box_data[1] + ' ' + box_data[2] + ' ' + box_data[3] + ' ' + box_data[4] + '\n'
                normal_box += [l]
...
# python masking.py --data_dir ./data/original/train --output_dir ./data/new/train