...
class DefaultAug(ImgAug):
    def __init__(self, ):
        self.augmentations = iaa.Sequential([
            iaa.Sharpen((0.0, 0.1)),
            iaa.Affine(rotate=(-3, 3), translate_percent=(-0.1, 0.1), scale=(1.0, 1.5)),
            # iaa.AddToBrightness((-30, 10)),
            iaa.MultiplyBrightness((0.8, 0.95)),
            # iaa.AddToHue((-5, 5)),
    ...
    # def __call__(self, data):
    #     image, label = data
    #     image_aug = self.augmentations(images=[image])[0]
...
class FlipAug_tstl(ImgAug):
    ...
    #         image = self.augmentations(images=[image])[0]
    #         labels[..., 1] = image.shape[1] - labels[..., 1]
    #         left = labels[..., 0] == 0
    #         right = labels[..., 0] == 1
    #         labels[left, 0] = 1
    #         labels[right, 0] = 0
