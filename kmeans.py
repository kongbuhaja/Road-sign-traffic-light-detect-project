...
def load_objects():
    ...
    file_path = '/home/ubuntu/project2/data/new/train/Annotations/*.txt'
...
def get_anchors(k, image_size):
    boxes = load_objects()
    result = kmeans(boxes, k)
    avg_acc = avg_iou(boxes, result)*100
    print("Average accuracy: {:.2f}%".format(avg_acc))
    result = (np.array(sorted(np.array(result), key=lambda x: x[0]*x[1]), dtype=np.float32) * image_size).astype(np.int32)
    print("Anchors")
    print(result)
    return result
